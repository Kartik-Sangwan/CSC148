...
class Employee:
    ...
    payt: This employee's total salary received till now.
    """
    id_: int
    name: str
    payt: float
    personal_days: Dict[int, List[date]]
    ...
    def record_personal_days(self, day: date) -> None:
        """asd
        """
        year = day.year
        if year in self.personal_days:
            if len(self.personal_days[year]) == PERSONAL_DAYS:
                print('You are fired.')
            else:
                self.personal_days[year].append(day)
        else:
            self.personal_days[year] = [day]
...
class HourlyEmployee(Employee):
    ...
    def get_monthly_payment(self) -> float:
        ...
        62.5
        """
        # needs to be changed according to the leaves taken.
        curr_month = date.today().month
        curr_year = date.today().year
        pd_this_month = 0
        if curr_year in self.personal_days:
            for d in self.personal_days[curr_year]:
                if d.month == curr_month:
                    pd_this_month += 1
        hours_worked = max(0, self.hours_per_month - 8 * pd_this_month)
        return round(hours_worked * self.hourly_wage, 2)
