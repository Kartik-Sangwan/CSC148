...
def merge3(lst1: List, lst2: List, lst3: List) -> List:
    ...
    while index1 < len(lst1) and index2 < len(lst2):
        if lst1[index1] <= lst2[index2]:
            temp.append(lst1[index1])
            index1 += 1
        else:
            temp.append(lst2[index2])
            index2 += 1
    temp += lst1[index1:] + lst2[index2:]
    index3 = 0
    index4 = 0
    while index3 < len(temp) and index4 < len(lst3):
        if temp[index3] <= lst3[index4]:
            merged.append(temp[index3])
            index3 += 1
        else:
            merged.append(lst3[index4])
            index4 += 1
    return merged + temp[index3:] + lst3[index4:]
...
def kth_smallest(lst: List, k: int) -> Any:
    ...
    if k < 0 or k >= len(lst):
        ...
        pivot = lst[k]
        smaller, bigger = _partition(lst[:k] + lst[k + 1:], pivot)
        if k == 0 and len(smaller) == 0:
            ...
            new_k = k - len(smaller)
            return kth_smallest([pivot] + bigger, new_k)
