...
class Player:
    ...
    def __init__(self, name: str, current: int, minstep: int,
                 maxstep: int, goal: int) -> None:
        """k"""
        self.name = name
...
class StrategicPlayer(Player):
    ...
    def move(self) -> int:
        ...
        if self.current == 0:
            ...
            return (1 + 4 * (self.turn - 1)) - self.current
...
class UserPlayer(Player):
    ...
    def move(self) -> int:
        """ss"""
        self.turn += 1
        m = int(input(f'Enter your move:'))
        while not self.min_step <= m <= self.max_step:
            m = int(input(f'wrong entry please enter again:'))
        return m
...
def make_player(generic_name: str) -> Player:
    ...
    if generic_name == 'p1':
        return UserPlayer(name, 0, 1, 3, 21)
    else:
        return StrategicPlayer(name, 0, 1, 3, 21)
