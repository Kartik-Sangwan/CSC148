...
def find_runs(lst: list) -> List[Tuple[int, int]]:
    ...
    while run_end < len(lst):
        ...
        # can also be done using simple one if.
        while run_end != len(lst) and lst[run_end - 1] <= lst[run_end]:
            run_end += 1
    ...
    #         # this is an ascending run
    #         run_end += 1
    #     # elif lst[run_end - 1] > lst[run_end]:
    #     #     # this is an descending run
    #     #     run_end += 1
    #     else:
    #         runs.append((run_start, run_end))
    #         run_start = run_end
...
def timsort(lst: list) -> None:
    ...
    while len(runs) > 1:
        tup2 = runs.pop()
        tup1 = runs.pop()
        _merge(lst, tup1[0], tup1[1], tup2[1])
        new_run = (tup1[0], tup2[1])
        runs.append(new_run)
