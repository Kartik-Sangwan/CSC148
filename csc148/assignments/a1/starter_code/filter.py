...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        """
        # the lst_calls is highlighted check crash case.
        # TODO: Implement this method
        var = False
        for customer in customers:
            if customer.get_id() == filter_string:
                tup = customer.get_history()
                lst_calls = (tup[0]).extend(tup[1])
                var = True
        if var is True:
            return lst_calls
        else:
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        """
        # TODO: Implement this method
        # the xxx part can contain char or not.
        lst_calls = []
        first = filter_string[0]
        if first != 'L' and first != 'G':
            return data
        for call in data:
            if first == 'L':
                if call.duration < int(filter_string[1:]):
                    lst_calls.append(call)
            else:
                if call.duration > int(filter_string[1:]):
                    lst_calls.append(call)
        return lst_calls
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        """
        # TODO: Implement this method
        # these are original coordinates of the given map.
        # llc = -79.697878, 43.576959
        # urc = -79.196382, 43.799568
        # ulc = -79.196382, 43.576959
        # lrc = -79.697878, 43.799568
        # using if elif in the for loop we dont take a call twice which may
        # have both src,dst loc in the same rectangle.
        lst = []
        if _check_valid(filter_string) == False:
            return data
        cd = filter_string.split(',')
        if _check_valid(filter_string):
            for call in data:
                if cd[0] <= call.dst_loc[0] <= cd[2] and cd[1] <= call.dst_loc[1] <= cd[3]:
                    lst.append(call)
                elif cd[0] <= call.src_loc[0] <= cd[2] and cd[1] <= call.src_loc[1] <= cd[3]:
                    lst.append(call)
...
def _check_valid(filter_string: str) -> bool:
    """ ds
    """
    llc = -79.697878, 43.576959
    urc = -79.196382, 43.799568
    ...
    lat1, long1, lat2, long2 = float(lst[0]), float(lst[1]), float(lst[2]), float(lst[3])
    if (llc[0] <= long1 <= urc[0] and llc[1] <= lat1 <= urc[1]) and (
            llc[0] <= long2 <= urc[0] and llc[1] <= lat2 <= urc[1]):
                return True
